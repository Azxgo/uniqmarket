...
export default function ProductPage() {
    ...
    const fetchReviewsAndRating = async () => {
        const resAvg = await apiClient(`/api/rating/get/${prod.product_id}`);
        const avgData = await resAvg.json();
        setAvgRating(avgData.avg_rating);

        const resReviews = await apiClient(`/api/rating/getReviews/${prod.product_id}`);
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            const res = await apiClient(`/api/rating/getReviews/${prod.product_id}`);
            const data = await res.json();
            setReviews(data);
        };

        const checkPurchase = async () => {
            const res = await apiClient(`/api/rating/checkPurchase/${prod.product_id}`, {
                credentials: "include"
            })
            const data = await res.json();
