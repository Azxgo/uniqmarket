...
export const CartProvider = ({ children }) => {
    ...
    const fetchCart = async () => {
        try {
            setIsloading(true)
            const res = await apiClient("/api/cart", { credentials: "include" });
    ...
    const removeFromCart = async (product) => {
        try {
            const res = await apiClient("/api/cart/remove", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                credentials: "include",
    ...
    const resetCart = async () => {
        ...
        try {
            const response = await apiClient("/api/cart/reset", {
                method: "POST",
                credentials: "include"
            });
    ...
    const buyProducts = async () => {
        ...
        if (!confirmed) return
        try {
            const response = await apiClient("/api/cart/buy", {
                method: "POST",
                credentials: "include"
            });
