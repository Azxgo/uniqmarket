...
export default function Cart() {
  const { cartProducts, addToCart, removeFromCart, resetCart, buyProducts, isLoading } = useCartContext();
      ...
      <div className="grid grid-cols-1 md:grid-cols-[auto_400px] gap-8 items-start">
            ...
            cartProducts.length === 0 ? (
              <div className="flex flex-col justify-center items-center h-100 gap-4">
                ...
                <div className="flex flex-col justify-center">
                  <h1>No se ha agregado ningún producto al carrito</h1>
                  <button>Ir a la tienda</button>
                </div>
              </div>
            ) : (
              cartProducts.map(({ product, quantity }) => {
                const stockRemaining = Math.max((product?.stock || 0) - (quantity || 0), 0);
                console.log(stockRemaining)

                return (
                  <div
                    key={product.product_id}
                    className="flex p-4 w-full border-b-2 border-gray-300 gap-4 items-center"
                  >
                    <div className="flex">
                    ...
                    <div className="flex flex-col h-full w-full justify-center">
                      ...
                      <span className="text-sm text-gray-500">Stock restante: {stockRemaining}</span>
                    </div>

                    <div className="flex flex-row h-full max-w-150 justify-end items-center gap-4">
                      <button
                        className="bg-zinc-900 hover:bg-zinc-700 text-white rounded-md p-1 w-8 h-8 cursor-pointer"
        ...
        <div className="flex flex-col bg-white rounded-md shadow-md p-5">
          ...
          <div className="flex flex-col py-3 gap-2">
            <button
              className="bg-zinc-900 hover:bg-zinc-700 text-white rounded-md p-1 w-full h-10 cursor-pointer"
              onClick={buyProducts}
            >
              Comprar
            </button>
